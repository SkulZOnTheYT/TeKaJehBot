...
export class mongoDB {
  ...
  async read() {
    ...
    this._data = await this._model.findOne({}).exec()
    if (!this._data) {
      this.data = {}
  ...
  write(data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this._data) {
        resolve(new this._model({ data }).save());
      } else {
        this._model.findById(this._data._id).exec().then(docs => {
            if (!docs) {
              return reject('Dokumen tidak ditemukan');
            }
            if (!docs.data) {
              docs.data = {};
            }
...
export const mongoDBV2 = class MongoDBV2 {
  ...
  async read() {
    ...
    this.lists = await this.list.findOne({}).exec()
    if (!this.lists?.data) {
    ...
    await Promise.all(this.lists.data.map(async ({ name }) => {
      ...
      let collection
      try {
        collection = _model(name, new Schema({ data: Array })).exec();
      } catch (e) {
        console.error(e)
        try {
          collection = _model(name).exec();
        } catch (e) {
          garbage.push(name)
          console.error(e)
        }
      ...
      if (collection) {
        ...
        const collectionsData = await collection.find({}).exec();
    ...
    try {
      ...
      let del = await this.list.findById(this.lists._id).exec()
      ...
      await Promise.all(collections.map(async (key) => {
        ...
        if (index !== -1) {
          ...
          if (Object.keys(data[key]).length > 0) {
            await doc.deleteMany().exec();
            await doc.insertMany(Object.entries(data[key]).map(v => ({ data: v }))).exec();            
          }
...
this.list.findById(this.lists._id).exec()
        .then(doc => {
          if (!doc) return reject('Dokumen tidak ditemukan');
          doc.data = listDoc;
          return doc.save();
        })
        .then(() => {
